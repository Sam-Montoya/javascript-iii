...
let totalPopulation = populations.reduce(function(prev, current){
    return prev + current;
});
...
let ordersTotal = orders.map((element, index) =>{
...
let bobsTotal = purchases.reduce((prev, current, index, arr) =>{
  var total;
    if(arr[index].owner === "Bob"){
      total = prev + arr[index].price;
    }
    else{
      return prev;
    }
